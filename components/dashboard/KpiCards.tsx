...
function KpiCard({ id, label, rawValue, unit, delta, icon: Icon, delay, decimals = 0, accentColor, glowColor }: KpiCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        <span className="text-[13px] font-medium tracking-wide" style={{ color: "rgba(255,255,255,0.6)" }}>{label}</span>
      ...
      <div className="flex items-baseline gap-1.5 relative z-10">
        <span className="text-[34px] font-light tracking-tight text-white text-glow-blue font-geist leading-none">
          {display}
        </span>
      ...
      <div className="mt-4 pt-3 border-t border-white/10 relative z-10">
        <span className="text-[12px] font-normal" style={{ color: "rgba(255,255,255,0.4)" }}>{delta}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <KpiCard id="kpi-total-courses" label="총 강좌 수" rawValue={totalCourses} unit="개" delta="필터링된 전체 강좌"
        icon={BookOpen} delay={0} accentColor="#A8D8F0" glowColor="rgba(168,216,240,0.4)" />
      <KpiCard id="kpi-avg-enroll" label="평균 수강률" rawValue={avgEnrollRate} unit="%" delta="수강정원 대비 수강인원"
        icon={BarChart2} delay={80} decimals={1} accentColor="#64B4FF" glowColor="rgba(100,180,255,0.4)" />
